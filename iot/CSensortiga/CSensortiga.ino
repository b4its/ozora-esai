 ...
 *    - Relay mesin via GPIO 26 (HIGH=ON, LOW=OFF)
 *    - Auto-matikan mesin via sterile-check API ketika air jernih
 *    - Dashboard web bisa nyalakan ulang mesin via /api/device/control/
...
#define MACHINE_RELAY_PIN       26   // Pin relay/buzzer mesin — HIGH = ON, LOW = OFF
...
bool machineOn              = true;   // State mesin: true = ON, false = OFF (steril)
...
// ============================================================
//  FUNGSI: Kontrol mesin (relay)
// ============================================================
void machineSetOn() {
  machineOn = true;
  digitalWrite(MACHINE_RELAY_PIN, HIGH);
  Serial.println(F("[Machine] ON"));
  i2cLedOn();
}
...
void machineSetOff() {
  ...
  Serial.println(F("[Machine] OFF — Air sudah jernih/steril!"));
  // Notifikasi visual: blink cepat 5x lalu mati
  i2cLedBlink(5, 80);
  i2cLedOff();
  if (lcd) {
    lcd->clear();
    lcd->setCursor(0, 0); lcd->print("AIR JERNIH!");
    lcd->setCursor(0, 1); lcd->print("Mesin MATI");
  }
}
...
void sendHeartbeat() {
  ...
  if (code == 200 || code == 201) {
    ...
    // Baca target_status dan is_sterile dari response
    String resp = http.getString();
    StaticJsonDocument<256> respDoc;
    if (!deserializeJson(respDoc, resp)) {
      bool target = respDoc["target_status"] | true;
      bool sterile = respDoc["is_sterile"]   | false;
      if (sterile && machineOn) {
        machineSetOff();
      } else if (target && !machineOn && !sterile) {
        machineSetOn();
      }
    }
  } else {
    Serial.printf("[Heartbeat] Status: %d\n", code);
    lcdPrintf(0, "Heartbeat FAIL");
  }
  http.end();
}
...
//  Selalu buka portal setiap boot — WiFi credentials tidak
//  disimpan permanen. Token tetap dimuat dari LittleFS.
// ============================================================
void startWiFiManager(bool forcePortal = false) {
  ...
  // Hapus WiFi credentials lama agar portal selalu muncul.
  // Token TIDAK dihapus — diisi ulang dari LittleFS (loadConfig).
  wm.resetSettings();
  ...
  bool connected = wm.startConfigPortal(AP_SSID, AP_PASSWORD);
  ...
  // Ambil token dari form portal; simpan ke flash hanya jika berubah
  ...
  // ── 2. Kirim ke sterile-check, baca respons mesin ──
  {
    ...
    if (code == 200) {
      ...
      if (!deserializeJson(respDoc, resp)) {
        bool target       = respDoc["target_status"]  | true;
        bool sterile      = respDoc["is_sterile"]     | false;
        bool justTriggered = respDoc["just_triggered"] | false;
        ...
        if (justTriggered || (sterile && machineOn)) {
          machineSetOff();
        } else if (target && !machineOn && !sterile) {
          machineSetOn();
        }
...
void setup() {
  ...
  // Relay mesin — default ON saat boot
...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Hanya kirim data sensor kalau mesin sedang ON
    if (machineOn && (now - lastSensor >= SEND_INTERVAL)) {
      sendSensorData();
      lastSensor = now;
    }
